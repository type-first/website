...
export const metadata = {
  title: 'Type Explorer — Multi-file Starter',
  description: 'Multi-file starter scenario with modules',
};

async function loadLocalScenario(): Promise<ExplorerFile[]> {
  const baseDir = path.join(process.cwd(), 'app', 'labs', 'type-explorer', 'scenarios', 'multi-file-starter', 'src');
...
export default async function MultiFileStarterScenarioPage() {
  const files = await loadLocalScenario();
  return (
    <div className="max-w-7xl mx-auto px-6 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Type Explorer</h1>
        <p className="text-gray-600 mt-1">Multi-file starter (modules)</p>
      </div>
