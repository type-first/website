...
function getFeedback() {
  return Promise.resolve([
    { name: 'Alice', emoji: '😍', score: 10 },
    { name: 'Bob', emoji: '😊', score: 8 }
  ])
}

const feedback0 = getFeedback()
...
jump in with hands-on examples
*/

// here's a phantom type - no runtime cost

type Email = string & { __brand: 'Email' }
type UserId = string & { __brand: 'UserId' }

const email = t_<Email>()
const userId = t_<UserId>()

// they look the same but are different types

no_<$Equal<Email, UserId>>() // ✓

// @ts-expect-error ✓
// type 'Email' is not assignable to type 'UserId'
const mixup: UserId = email

// here are some type comparisons
...
// let's build a user system

type UserEmail = string & { __brand: 'UserEmail' }
type AdminId = string & { __brand: 'AdminId' }

const userEmail = t_<UserEmail>()
const adminId = t_<AdminId>()

no_<$Equal<UserEmail, AdminId>>() // ✓
...
function validateUser2<T extends User2>(userParam: T): T {
  return userParam
}

// here's an API response pattern
...
type UserResponse = ApiResponse<User2>

// test the structure

const response = t_<UserResponse>()

yes_<$Extends<UserResponse['data'], User2>>() // ✓

// let's test some types

const userTypeTest = test_('User type structure', () => {
  yes_<$Extends<User2, { email: UserEmail }>>()
  yes_<$Extends<User2, { name: string }>>()
  
  no_<$Equal<User2, { id: string }>>()
  
  return t_<User2>()
})

// here's a type-level proof
...
// phantom types - types without runtime shape

// think of them as compile-time labels
type Brand<T, U> = T & { __brand: U }

// examples:
type UserId_0 = Brand<string, 'UserId'>
type OrderId = Brand<string, 'OrderId'>

// 📍 different types even though they're both strings

const userId_0 = 'user123' as UserId_0
const orderId = 'order456' as OrderId

// @ts-expect-error - can't assign different phantom types
let wrongAssign: UserId_0 = orderId

// @ts-expect-error - can't assign to wrong phantom type  
let wrongAssign2: OrderId = userId_0

// 💪 typist gives us type-safe validation

const phantomTest = example_('Phantom type safety', () => {
  yes_<$Equal<Brand<string, 'A'>, Brand<string, 'A'>>>()
  
  no_<$Equal<Brand<string, 'A'>, Brand<string, 'B'>>>()
  
  return t_<UserId_0>()
})

// here's the old problematic way:

let userName2 = 'john@example.com'
let userId2 = 'user-123'
let sessionId2 = 'session-abc'

// ❌ all just strings - no safety
function createSession(userId: string, sessionId: string) {
  // oops! passed arguments in wrong order  
...
type SafeSessionId = Branded<string, 'SessionId'>

// ✨ now we have type safety
function createSafeSession(userId: SafeUserId, sessionId: SafeSessionId) {
  return { userId, sessionId }
}

const safeUserId2 = 'user-123' as SafeUserId
const safeSessionId2 = 'session-abc' as SafeSessionId

// wrong order is still risky with original function
createSession(safeSessionId2 as any, safeUserId2 as any)

// ✓ correct order works
createSession(safeUserId2 as any, safeSessionId2 as any)
...
// usage
const safeUserId3 = createUserId("user_123");
const safeEmail = createEmail("user@example.com");

const safeUser = createUserTypeSafe(safeUserId3, safeEmail, "John"); // ✅ Correct order enforced
...
// permission systems with phantoms

type Permission = 'read' | 'write' | 'admin'

type SecureData<P extends Permission> = {
  data: string
  readonly __permission: P
}

type PermissionUser<P extends Permission> = {
  id: UserId
  name: string
  readonly __permission: P
}
...
function accessAdminData<P extends Permission>(
  user: PermissionUser<P>,
  data: SecureData<'admin'>
): P extends 'admin' ? string : never {
...
// read user can't access admin data  
const denied = accessAdminData(readUser as any, adminData)

// ✓ admin user can access admin data
const allowed = accessAdminData(adminUser, adminData)
...
export {
  ...
  validateUser2, validateConfig, isValidUserId
};
